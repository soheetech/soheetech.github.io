...
- 플랫폼에 독립적(=아키텍처 중립적) : 운영체제(OS)와 관계 없이 Java플랫폼(실행환경)만 설치되어 있으면 동일 코드로 동작함(=이식성이 높음)
...
*바이트 코드 : Java 소스를 컴파일한 결과물로 확장자는 .class이며 클래스 파일이라고도 함, 자바 플랫폼의 JVM에서 실행 가능한 코드로 컴퓨터상에서 직접적으로 실행될 수 있는 기계어가 아님.*<br /><br />
...
<h5>Java 애플릿</h5>
- JVM을탑재한 웹 브라우저에서 실행되는 Java 프로그램
